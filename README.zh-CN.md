...
**📖 源码中文导读（推荐先读，v1 已覆盖主链路）**

上游文档极少、信息几乎全在代码里，因此除文档翻译外，本仓库额外提供**源码导读**，按代码讲清架构与设计取舍（不逐行翻译）：

| 篇目 | 内容 |
| --- | --- |
| [导读索引](docs/导读/README.md) | v1 覆盖范围、系统全景图、模块速查表与阅读顺序 |
| [01 请求生命周期与流水线框架](docs/导读/01-请求生命周期与流水线框架.md) | For You 请求全链路；`candidate-pipeline` 的 10 个阶段抽象 |
| [02 home-mixer 核心推荐链路](docs/导读/02-home-mixer核心推荐链路.md) | 7 路召回 / 17 查询补水 / 12 候选补水 / 18 过滤器 / 3 打分器，含真实权重表 |
| [03 关键设计模式与工程取舍](docs/导读/03-关键设计模式与工程取舍.md) | Prod-Mock 注入、Feature Switch 参数化、分级处理、副作用解耦、可观测性 |
| [04 thunder 网内召回内存存储](docs/导读/04-thunder网内召回内存存储.md) | 内存索引设计、每维度硬上限、删除墓碑、快照+增量冷启动 |
| [05 phoenix 模型导读](docs/导读/05-phoenix模型导读.md) | 双塔召回与排序模型架构、哈希嵌入、损失函数、训练与运行器 |
| [06 simclusters 社群聚类导读](docs/导读/06-simclusters社群聚类导读.md) | 用户-社群兴趣表示、近似余弦 ANN 服务、离线物化流程 |
| [07 内容治理规则引擎导读](docs/导读/07-内容治理规则引擎导读.md) | `.bot` 规则 DSL、编译链路、运行时动作分级、scarecrow 实时打标 |
...
| `home-mixer/` | 构建 For You 信息流的核心服务（无英文 README 源，**已有源码导读**） | 📖 导读可读 |
| `candidate-pipeline/` | 推荐流水线通用框架（无英文 README 源，**已有源码导读**） | 📖 导读可读 |
